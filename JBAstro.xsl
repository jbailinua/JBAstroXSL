...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template name="Start">
		<xsl:choose>
			<xsl:when test="b:Version">
				<xsl:text>2021.0</xsl:text>
			</xsl:when>
			
			<xsl:when test="b:OfficeStyleKey">
				<xsl:text>JBAstro</xsl:text>
			</xsl:when>
	...
	<xsl:template name ="BibDisplayJournalName">
		...
		<xsl:if test ="$cJournalName!=0">
		    <xsl:variable name="journal0" select="b:JournalName" />
		    <xsl:choose>
		        <xsl:when test="$journal0='\aj'">
		            <xsl:text>AJ</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\araa'">
		            <xsl:text>ARA&amp;A</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\apj'">
		            <xsl:text>ApJ</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\apjl'">
		            <xsl:text>ApJL</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\apjs'">
		            <xsl:text>ApJS</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\aap'">
		            <xsl:text>A&amp;A</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\aapr'">
		            <xsl:text>A&amp;AR</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\aaps'">
		            <xsl:text>A&amp;AS</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\mnras'">
		            <xsl:text>MNRAS</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\pra'">
		            <xsl:text>PhysRevA</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\prb'">
		            <xsl:text>PhysRevB</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\prc'">
		            <xsl:text>PhysRevC</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\prd'">
		            <xsl:text>PhysRevD</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\prl'">
		            <xsl:text>PhysRevL</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\pasp'">
		            <xsl:text>PASP</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\pasj'">
		            <xsl:text>PASJ</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\nat'">
		            <xsl:text>Nature</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\na'">
		            <xsl:text>New Astronomy</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\pasa'">
		            <xsl:text>PASA</xsl:text>
		        </xsl:when>
		        <xsl:when test="$journal0='\rmxaa'">
		            <xsl:text>RMxAA</xsl:text>
		        </xsl:when>
		        <xsl:otherwise>
		            <xsl:value-of select="$journal0"/>
		        </xsl:otherwise>
		    </xsl:choose>
			<xsl:choose>
				<xsl:when test="$cVolume!=0 or $cIssue!=0 or $cPages!=0 or $cYear!=0">
				...
				<xsl:element name="tr">
					...
					<xsl:element name="td">
						...
						<xsl:element name="p">
							...
							<xsl:choose>
							<xsl:when test="count(b:ThesisType)>0">
							    <xsl:choose>
							    <xsl:when test="b:ThesisType='phd'">
